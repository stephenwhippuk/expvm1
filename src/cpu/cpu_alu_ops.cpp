...
void Cpu::execute_add_operation(byte_t opcode, const std::vector<byte_t>& params) {
        switch(opcode) {
            case OPCODE_ADD_REG_W:
                {
                    ...
                    alu_->add(reg->get_accessor()->get_value());
                }
                break;
            ...
            case OPCODE_ADH_REG_B:
                {
                    auto reg = get_register_by_code(params[0]);
                    alu_->add_byte(reg->get_accessor()->get_high_byte());
                }
                break;
            case OPCODE_ADL_REG_B:
                {
                    auto reg = get_register_by_code(params[0]);
                    alu_->add_byte(reg->get_accessor()->get_low_byte());
                }
                break;
            default:
...
void Cpu::execute_sub_operation(byte_t opcode, const std::vector<byte_t>& params) {
    switch(opcode) {
        ...
        case OPCODE_SUB_REG_W:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->sub(reg->get_accessor()->get_value());
            }
            break;
        ...
        case OPCODE_SBH_REG_B:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->sub_byte(reg->get_accessor()->get_high_byte());
            }
            break;
        case OPCODE_SBL_REG_B:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->sub_byte(reg->get_accessor()->get_low_byte());
            }
            break;
        default:
...
void Cpu::execute_mul_operation(byte_t opcode, const std::vector<byte_t>& params) {
    switch(opcode) {
        ...
        case OPCODE_MUL_REG_W:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->mul(reg->get_accessor()->get_value());
            }
            break;
        ...
        case OPCODE_MLH_REG_B:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->mul_byte(reg->get_accessor()->get_high_byte());
            }
            break;
        case OPCODE_MLL_REG_B:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->mul_byte(reg->get_accessor()->get_low_byte());
            }
            break;
        default:
...
void Cpu::execute_div_operation(byte_t opcode, const std::vector<byte_t>& params) {
    switch(opcode) {
        ...
        case OPCODE_DIV_REG_W:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->div(reg->get_accessor()->get_value());
            }
            break;
        ...
        case OPCODE_DVH_REG_B:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->div_byte(reg->get_accessor()->get_high_byte());
            }
            break;
        case OPCODE_DVL_REG_B:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->div_byte(reg->get_accessor()->get_low_byte());
            }
            break;
        default:
...
void Cpu::execute_rem_operation(byte_t opcode, const std::vector<byte_t>& params) {
    switch(opcode) {
        ...
        case OPCODE_REM_REG_W:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->rem(reg->get_accessor()->get_value());
            }
            break;
        ...
        case OPCODE_RMH_REG_B:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->rem_byte(reg->get_accessor()->get_high_byte());
            }
            break;
        case OPCODE_RML_REG_B:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->rem_byte(reg->get_accessor()->get_low_byte());
            }
            break;
        default:
...
void Cpu::execute_and_operation(byte_t opcode, const std::vector<byte_t>& params) {
    switch(opcode) {
        ...
        case OPCODE_AND_REG_W:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->bit_and(reg->get_accessor()->get_value());
            }
            break;
        ...
        case OPCODE_ANH_REG_B:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->bit_and_byte(reg->get_accessor()->get_high_byte());
            }
            break;
        case OPCODE_ANL_REG_B:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->bit_and_byte(reg->get_accessor()->get_low_byte());
            }
            break;
        default:
...
void Cpu::execute_or_operation(byte_t opcode, const std::vector<byte_t>& params) {
    switch(opcode) {
        ...
        case OPCODE_OR_REG_W:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->bit_or(reg->get_accessor()->get_value());
            }
            break;
        ...
        case OPCODE_ORH_REG_B:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->bit_or_byte(reg->get_accessor()->get_high_byte());
            }
            break;
        case OPCODE_ORL_REG_B:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->bit_or_byte(reg->get_accessor()->get_low_byte());
            }
            break;
        default:
...
void Cpu::execute_xor_operation(byte_t opcode, const std::vector<byte_t>& params) {
    switch(opcode) {
        ...
        case OPCODE_XOR_REG_W:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->bit_xor(reg->get_accessor()->get_value());
            }
            break;
        ...
        case OPCODE_XOH_REG_B:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->bit_xor_byte(reg->get_accessor()->get_high_byte());
            }
            break;
        case OPCODE_XOL_REG_B:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->bit_xor_byte(reg->get_accessor()->get_low_byte());
            }
            break;
        default:
...
void Cpu::execute_not_operation(byte_t opcode, const std::vector<byte_t>& params) {
    switch(opcode) {
        case OPCODE_NOT_IMM_W:
            {
                word_t value = combine_bytes_to_word(params[0], params[1]);
                // NOT takes a value and stores result in AX
                // Current ALU::bit_not() operates on AX, so set AX first
                alu_->get_accumulator()->set_value(value);
                alu_->bit_not();
            }
            break;
        case OPCODE_NOT_REG_W:
            {
                auto reg = get_register_by_code(params[0]);
                // NOT takes register value and stores result in AX
                alu_->get_accumulator()->set_value(reg->get_accessor()->get_value());
                alu_->bit_not();
            }
            break;
        case OPCODE_NOTB_IMM_B:
            {
                // NOT byte: Set AX to immediate byte, NOT it
                alu_->get_accumulator()->set_value(static_cast<word_t>(params[0]));
                alu_->bit_not();
            }
            break;
        case OPCODE_NOTH_REG_B:
            {
                auto reg = get_register_by_code(params[0]);
                // NOT high byte: Set AX to high byte, NOT it
                alu_->get_accumulator()->set_value(static_cast<word_t>(reg->get_accessor()->get_high_byte()));
                alu_->bit_not();
            }
            break;
        case OPCODE_NOTL_REG_B:
            {
                auto reg = get_register_by_code(params[0]);
                // NOT low byte: Set AX to low byte, NOT it
                alu_->get_accumulator()->set_value(static_cast<word_t>(reg->get_accessor()->get_low_byte()));
                alu_->bit_not();
            }
            break;
        default:
            throw lvm::runtime_error("Invalid not opcode");
    }
}
...
void Cpu::execute_shift_operation(byte_t opcode, const std::vector<byte_t>& params) {
    switch(opcode) {
        ...
        case OPCODE_SHL_REG_W:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->shl(reg->get_accessor()->get_value());
            }
            break;
        ...
        case OPCODE_SLH_REG_B:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->shl(reg->get_accessor()->get_high_byte());
            }
            break;
        case OPCODE_SLL_REG_B:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->shl(reg->get_accessor()->get_low_byte());
            }
            break;
        ...
        case OPCODE_SHR_REG_W:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->shr(reg->get_accessor()->get_value());
            }
            break;
        ...
        case OPCODE_SHRH_REG_B:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->shr(reg->get_accessor()->get_high_byte());
            }
            break;
        case OPCODE_SHRL_REG_B:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->shr(reg->get_accessor()->get_low_byte());
            }
            break;
        default:
...
void Cpu::execute_rotate_operation(byte_t opcode, const std::vector<byte_t>& params) {
    switch(opcode) {
        ...
        case OPCODE_ROL_REG_W:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->rol(reg->get_accessor()->get_value());
            }
            break;
        ...
        case OPCODE_ROLH_REG_B:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->rol(reg->get_accessor()->get_high_byte());
            }
            break;
        case OPCODE_ROLL_REG_B:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->rol(reg->get_accessor()->get_low_byte());
            }
            break;
        ...
        case OPCODE_ROR_REG_W:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->ror(reg->get_accessor()->get_value());
            }
            break;
        ...
        case OPCODE_RORH_REG_B:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->ror(reg->get_accessor()->get_high_byte());
            }
            break;
        case OPCODE_RORL_REG_B:
            {
                auto reg = get_register_by_code(params[0]);
                alu_->ror(reg->get_accessor()->get_low_byte());
            }
            break;
        default:
        ...
        case OPCODE_CMP_REG_REG:
            {
                ...
                AX->get_accessor()->set_value(reg1->get_accessor()->get_value());
                alu_->cmp(reg2->get_accessor()->get_value());
            }
            break;
        case OPCODE_CMP_REG_IMM_W:
            {
                ...
                auto reg = get_register_by_code(params[0]);
                AX->get_accessor()->set_value(reg->get_accessor()->get_value());
        ...
        case OPCODE_CPH_REG_REG:
            {
                ...
                auto reg1 = get_register_by_code(params[0]);
                auto reg2 = get_register_by_code(params[1]);
                AX->get_accessor()->set_value(reg1->get_accessor()->get_high_byte());
                alu_->cmp_byte(reg2->get_accessor()->get_high_byte());
            }
            break;
        case OPCODE_CPH_REG_IMM_B:
            {
                ...
                auto reg = get_register_by_code(params[0]);
                AX->get_accessor()->set_value(reg->get_accessor()->get_high_byte());
        ...
        case OPCODE_CPL_REG_REG:
            {
                ...
                auto reg1 = get_register_by_code(params[0]);
                auto reg2 = get_register_by_code(params[1]);
                AX->get_accessor()->set_value(reg1->get_accessor()->get_low_byte());
                alu_->cmp_byte(reg2->get_accessor()->get_low_byte());
            }
            break;
        case OPCODE_CPL_REG_IMM_B:
            {
                ...
                auto reg = get_register_by_code(params[0]);
                AX->get_accessor()->set_value(reg->get_accessor()->get_low_byte());
                alu_->cmp_byte(params[1]);
            }
            break;
        default:
