...
/**
 * Creates a Pendragon VM binary file with data segment
 * 
 * Data segment contains: "Hello, World!"
 * Code segment contains: HALT instruction
 */
...
int main(int argc, char* argv[]) {
    ...
    std::string program_name = "HelloWorldWithData";
    ...
    // Write data segment (contains "Hello, World!" string)
    std::vector<byte_t> data;
    for (char c : data_string) {
        data.push_back(static_cast<byte_t>(c));
    }
    ...
    std::cout << "  Data segment: " << data.size() << " bytes ('" << data_string << "')" << std::endl;
